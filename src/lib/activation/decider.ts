...
export class ActivationDecider {
   ...
   */
  async shouldRespond(context: ActivationContext): Promise<ActivationDecision> {
    ...
    const prompt = ACTIVATION_PROMPT.replace('{name}', schruteConfig.name)
      .replace(
        '{aliases}',
        schruteConfig.aliases && schruteConfig.aliases.length > 0
      ...
      .replace(
        '{responsibilities}',
        schruteConfig.areas_of_responsibility && schruteConfig.areas_of_responsibility.length > 0
          ? schruteConfig.areas_of_responsibility.join(', ')
          : 'none'
      )
      .replace(
        '{expertise}',
        schruteConfig.expertise_keywords && schruteConfig.expertise_keywords.length > 0
    ...
    try {
      const decision = await this.client.promptJson<ActivationDecision>(prompt, {
   ...
   */
  async shouldRespondBatch(
    emails: Email[],
    schruteConfig: SchruteConfig,
    threads?: Map<string, EmailThread>
  ): Promise<Map<string, ActivationDecision>> {
    const decisions = new Map<string, ActivationDecision>()

    for (const email of emails) {
      const threadHistory = threads?.get(email.thread_id)?.messages.filter(
        (m) => new Date(m.timestamp) < new Date(email.timestamp)
      )
...
export function createActivationDecider(): ActivationDecider {
