...
# Clean stale build artifacts (automatically runs before tests)
npm run clean
```

**Important:** Stale `.js` and `.d.ts` files in `src/` can interfere with tests. The project includes:
- `.gitignore` excludes `src/**/*.js` and `src/**/*.d.ts` to prevent committing build artifacts
- `npm run clean` removes any stale compiled files from `src/`
- `npm test` automatically runs `clean` via the `pretest` script
...
- `src/**/*.js` and `src/**/*.d.ts` files (gitignored, generated by tsc, cleaned automatically)
