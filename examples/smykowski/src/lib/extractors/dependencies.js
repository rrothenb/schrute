...
export class DependencyExtractor {
     ...
     */
    extractDependencies(text, currentIssue) {
        const dependencies = [];
     ...
     */
    getBlockers(issueNumber, dependencies) {
        return dependencies
            .filter(dep => dep.status === 'active' && dep.blocked_issue_number === issueNumber)
            .map(dep => dep.blocker_issue_number);
    }
    /**
     * Try to extract current issue number from email
     */
    extractCurrentIssue(email) {
