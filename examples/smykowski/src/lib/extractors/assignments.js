...
export class AssignmentLogic {
     ...
     */
    suggestAssignee(item, teamMembers) {
        ...
        const available = teamMembers.filter(m => !m.vacation_schedule?.is_on_vacation);
     ...
     */
    suggestReassignments(overloadedMember, teamMembers, issues) {
        const reassignments = [];
        // Get items assigned to overloaded member
        const assignedItems = issues.filter(i => i.assignee_github === overloadedMember.github_username);
        // Get available members (sorted by workload, ascending)
        const available = teamMembers
            .filter(m => m.github_username !== overloadedMember.github_username &&
     ...
     */
    calculateWorkloadBalance(teamMembers) {
     ...
     */
    getOverloadedMembers(teamMembers, threshold = 2.0) {
        const { avgWorkload } = this.calculateWorkloadBalance(teamMembers);
        return teamMembers.filter(m => avgWorkload > 0 && m.current_workload / avgWorkload >= threshold);
    }
}
