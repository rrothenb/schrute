...
export class WikiManager {
     ...
     */
    async updateSection(pageName, sectionTitle, newContent) {
     ...
     */
    async appendToPage(pageName, content) {
        const existing = await this.getPage(pageName);
        if (!existing) {
    ...
    replaceSectionContent(content, sectionTitle, newContent) {
        const sectionRegex = new RegExp(`(^##\\s+${this.escapeRegex(sectionTitle)}\\s*$)([\\s\\S]*?)(?=^##\\s|$)`, 'gm');
